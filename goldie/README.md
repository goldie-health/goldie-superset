...
```bash
docker build --platform linux/amd64 --target lean -t prod-goldie-superset .
```

```bash
aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin <your-ecr-repository-url>
docker tag prod-goldie-superset:latest <your-ecr-repository-url>/prod-goldie-superset:latest
docker push <your-ecr-repository-url>/prod-goldie-superset:latest
```